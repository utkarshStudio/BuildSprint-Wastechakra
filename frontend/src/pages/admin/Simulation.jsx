...
export default function AdminSimulation() {
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-headline-md text-headline-md text-primary font-bold">Simulation</h1>
        <p className="font-label-sm text-label-sm text-on-surface-variant mt-1">City-wide MRF network simulation & processing line twin.</p>
      </div>
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-title-md text-title-md text-primary font-bold">Network Live Parameters</h2>
        ...
        <p className="font-label-sm text-label-sm text-on-surface-variant mb-5">
          Aggregated twin metrics across the MRF network. The architecture is ready to accept live industrial telemetry (input rate, composition, machine state, temperature, moisture, output flow) when facility sensors are connected.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { label: 'Network Input Rate', value: '18.4 t/h', pct: 62 },
            { label: 'Avg Composition', value: 'Mixed MSW', pct: 52 },
            { label: 'Machines Online', value: '7 / 8', pct: 87 },
            { label: 'Avg Temperature', value: '34°C', pct: 40 },
            { label: 'Avg Moisture', value: '42%', pct: 42 },
            { label: 'Output Recovery', value: '76%', pct: 76 },
          ].map((p) => (
