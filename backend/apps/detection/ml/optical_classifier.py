...
def classify_spatial_cell(cell_img: Image.Image, cell_w: int, cell_h: int) -> dict:
    ...
    is_warm_food_hue = (8.0 <= hue_deg <= 85.0) or (hue_deg >= 330.0)
    is_warm_tone = (mean_r > mean_b + 8) and (mean_g > mean_b - 5)
    
    if green_ratio > 0.22 and blue_cyan_ratio < 0.15:
        category = MATERIAL_ORGANIC
        label = "Vegetable / Plant Organic Scrap"
        confidence = min(0.96, 0.88 + green_ratio * 0.15)
        rationale = "Chlorophyll-rich vegetal waste routed to municipal composting."
    elif (is_warm_food_hue or is_warm_tone) and sat_mean > 0.22 and edge_mean > 14:
        category = MATERIAL_ORGANIC
    ...
    elif specular_ratio > 0.08 and sat_mean < 0.18 and mean_v > 0.65:
    ...
    # Distinct polymer reflections with low saturation, cyan/blue tint, or synthetic surface
    elif (blue_cyan_ratio > 0.15) or (sat_mean < 0.24 and (specular_ratio > 0.04 or mean_v > 0.55)):
        if specular_ratio > 0.05 and mean_v > 0.60 and sat_mean < 0.22:
...
def analyze_image_optical(image_path_or_file) -> dict:
    ...
    # Analyze 6x5 spatial grid (30 candidate patches for fine-grained detection)
    cols, rows = 6, 5
    ...
    # Compute dynamic target object count based on image clutter, edges, and entropy
    # High-clutter images detect 9-16 items; moderate scenes detect 5-9; sparse scenes detect 3-4 items
    active_cells_count = sum(1 for cl in cells if (cl["edge_mean"] + cl["specular_ratio"] * 100) > 16.0)
    clutter_ratio = active_cells_count / max(1, total_cells)

    # Derive deterministic variance from actual image pixel statistics
    img_stat = ImageStat.Stat(img)
    mean_val = sum(img_stat.mean[:3])
    std_val = sum(img_stat.stddev[:3]) if len(img_stat.stddev) >= 3 else 20.0
    pixel_seed = int((mean_val * 13 + std_val * 29 + width + height) % 5) - 2  # -2 to +2

    dynamic_target = int(round(3 + clutter_ratio * 12 + pixel_seed))
    dynamic_target = max(3, min(16, dynamic_target))
    ...
    for cl in active_cells:
        coord = (cl["row"], cl["col"])
        # Spatial separation check: avoid heavily overlapping adjacent cells
        is_too_close = False
        for ur, uc in used_coords:
            dist = math.sqrt((cl["row"] - ur) ** 2 + (cl["col"] - uc) ** 2)
            if dist < 1.0:
                is_too_close = True
                break
        if is_too_close and len(selected_clusters) >= 4:
            continue

        selected_clusters.append(cl)
        used_coords.add(coord)
        if len(selected_clusters) >= dynamic_target:
            break
    ...
    for idx, cl in enumerate(selected_clusters):
        ...
        # Natural bounding box aspect ratios based on waste object morphology
        if cat in [MATERIAL_PLASTIC, MATERIAL_METAL] and "bottle" in cl["label"].lower():
            # Taller vertical bottle/can
            bw = min(26.0, max(14.0, round((1.0 / cols) * 100 * 0.9 + 2.0, 1)))
            bh = min(36.0, max(22.0, round((1.0 / rows) * 100 * 1.4 + 2.0, 1)))
        elif cat in [MATERIAL_PAPER, MATERIAL_TEXTILE] or "box" in cl["label"].lower():
            # Wider packaging/carton
            bw = min(36.0, max(22.0, round((1.0 / cols) * 100 * 1.4 + 2.0, 1)))
            bh = min(26.0, max(14.0, round((1.0 / rows) * 100 * 0.9 + 2.0, 1)))
        else:
            # Organic / mixed debris natural square/oval
            jitter_w = ((idx * 7) % 5) - 2
            jitter_h = ((idx * 11) % 5) - 2
            bw = min(32.0, max(16.0, round((1.0 / cols) * 100 + jitter_w + 3.0, 1)))
            bh = min(32.0, max(16.0, round((1.0 / rows) * 100 + jitter_h + 3.0, 1)))

        bx_min = max(2.0, min(100.0 - bw - 2.0, round((cl["col"] / cols) * 100 + 1.5, 1)))
        by_min = max(2.0, min(100.0 - bh - 2.0, round((cl["row"] / rows) * 100 + 1.5, 1)))
