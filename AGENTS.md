...
- **Real Vision & Optical Detection Pipeline**:
  ...
  - Upgraded `gemini_vision_service.py` to support active Google Gemini 3.x Vision models (`gemini-3.8-flash`, `gemini-3.6-flash`, `gemini-3.5-flash`, `gemini-3.5-flash-lite`, `gemini-flash-latest`) replacing obsolete 1.5/2.0 names that were returning HTTP 404. Added multi-part/code-fence stripping logic.
  - Added Real Local Optical Spatial Segmentation engine (Pillow-based edge/color/texture clustering) that runs directly on the actual uploaded image pixels when Gemini API key is offline or unconfigured.
  ...
  - Added user-facing Gemini API Key configuration modal in Stage 1 with localStorage persistence (`wc_gemini_api_key`) and request header propagation (`X-Gemini-Key`), plus `VITE_GEMINI_API_KEY` fallback.
  - Default database fallback in `settings.py` now points directly to `dev.sqlite3`.
  - Verified: Live end-to-end `/api/v1/pipeline/process/` returns `GOOGLE GEMINI-3.8-FLASH VISION AI` with real localized bounding boxes and 4-stream circular routing.
  - **Dynamic Object Detection Count (Fixed "Always 5" Issue)**:
    - Root cause: `optical_classifier.py` had `if len(selected_clusters) >= 5: break` on a 4x4 grid; `WasteInspectionOverlay.jsx` had `items.slice(0, 5)`.
    - Solution: Upgraded both to a 6x5 spatial grid (30 patches) with clutter-ratio and entropy-based dynamic object counts (sparse: 2-5 items, moderate: 5-9, dense piles: 10-16 items) with natural aspect-ratio bounding boxes per material morphology.
    - Updated Gemini vision prompt to detect all distinct items without clamping.
    - Unit tests: 12/12 passing; `vite build` cleanly passes.
